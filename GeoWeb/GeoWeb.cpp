...
GeoWeb::GeoWeb(const InstanceInfo& info)
...
//    pGraphics->AttachControl(new ISVGKnobControl(masterArea.GetCentredInside(100), knobSVG, kParamGain)); /* TASK_02 */
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "GeoWeb", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void GeoWeb::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void GeoWeb::ProcessMidiMsg(const IMidiMsg& msg)
{
...
void GeoWeb::OnParamChange(int paramIdx)
{
