...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\GeoWeb Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\GeoWeb Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\GeoWeb manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\GeoWeb.vst3\Contents\\x86_64-win\GeoWeb.vst3",
      projectpath + "\\build-win\GeoWeb_x64.exe"  
    ]

  zipname = "GeoWeb-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\GeoWeb-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\GeoWeb-app_x64.pdb"  
  ]
